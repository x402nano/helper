...
__export(typescript_exports, {
  Helper: () => Helper,
  calculateBalanceAfterSend: () => calculateBalanceAfterSend,
...
var N = __toESM(require('nanocurrency'))
...
var ERROR_INSUFFICIENT_NANO_BALANCE = 'error_insufficient_nano_balance'
...
function calculateBalanceAfterSend({ currentBalance, amountToSend }) {
  const bnBalanceAfterSend = new bignumber_default(currentBalance).minus(amountToSend)
...
function validateNanoRpcUrl(url2, bypassNullCheck = false) {
  if (!bypassNullCheck && !url2) {
    throw new Error(`[${ERROR_CONFIG}] NANO_RPC_URL is not set`)
  }
  if (!URL.safeParse(url2).success) {
    throw new Error(`[${ERROR_CONFIG}] NANO_RPC_URL is not a valid URL`)
  }
}
function validateNanoWorkGenerationUrl(url2, bypassNullCheck = false) {
  if (!bypassNullCheck && !url2) {
...
var Helper = class {
   ...
   */
  setNanoRpcUrl(url2) {
    validateNanoRpcUrl(url2, true)
    this.config.NANO_RPC_URL = url2
   ...
   * @throws Error if the URL is invalid
   */
  setNanoWorkGenerationUrl(url2) {
    validateNanoWorkGenerationUrl(url2, true)
    this.config.NANO_WORK_GENERATION_URL = url2
  }
  /**
