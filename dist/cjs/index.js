...
__export(typescript_exports, {
  Helper: () => Helper,
  ensureNanoPrefix: () => ensureNanoPrefix,
  validateCustomWorkGenerator: () => validateCustomWorkGenerator,
  validateNanoAccountPrivateKey: () => validateNanoAccountPrivateKey,
  validateNanoRpcUrl: () => validateNanoRpcUrl,
  validateNanoWorkGenerationUrl: () => validateNanoWorkGenerationUrl
});
module.exports = __toCommonJS(typescript_exports);
var z = __toESM(require("zod"));
var import_blocks = require("nano-sdk/blocks");
var import_crypto = require("nano-sdk/crypto");
var import_types = require("nano-sdk/types");
...
var BLOCK_INFO_SUCCESS = z.object({
  block_account: (0, import_types.AccountString)(),
  amount: (0, import_types.RawAmountString)(),
  balance: (0, import_types.RawAmountString)(),
  height: z.ZodString,
  local_timestamp: z.ZodString,
  successor: (0, import_types.HashString)(),
  confirmed: z.union([z.literal("true"), z.literal("false")]),
  contents: (0, import_blocks.StateBlock)(),
  subtype: (0, import_types.SubtypeString)()
});
...
function getAccount({ config }) {
  return (0, import_crypto.deriveAccountFromPrivateKey)({ privateKey: config[import_typescript_common.NANO_ACCOUNT_PRIVATE_KEY_PROPERTY] });
}
function validateNanoRpcUrl(url, bypassNullCheck = false) {
  ...
  if (!import_typescript_common.URL.safeParse(url).success) {
...
function validateNanoAccountPrivateKey(privateKey, bypassNullCheck = false) {
  if (!bypassNullCheck && !privateKey) {
    throw new Error(`[${ERROR_CONFIG}] ${import_typescript_common.NANO_ACCOUNT_PRIVATE_KEY_PROPERTY} is not set`);
  }
  if (!import_typescript_common.HEX_64.safeParse(privateKey).success) {
    throw new Error(
      `[${ERROR_CONFIG}] ${import_typescript_common.NANO_ACCOUNT_PRIVATE_KEY_PROPERTY} is not of a valid format (should be 64 hexadecimal characters)`
    );
  }
}
function validateCustomWorkGenerator(workGenerator) {
  if (!import_typescript_common.WORK_GENERATOR.safeParse(workGenerator).success) {
...
var Helper = class {
  ...
  async generateSendBlock(params) {
    validateNanoRpcUrl(this.config.NANO_RPC_URL);
    validateNanoAccountPrivateKey(this.config[import_typescript_common.NANO_ACCOUNT_PRIVATE_KEY_PROPERTY]);
    ...
    if ("contents" in sourceAccountFrontierBlockInfo) {
      block = (0, import_blocks.createSendBlock)({
        amount: params.amount,
        destination: params.payTo,
        frontierBlock: sourceAccountFrontierBlockInfo.contents,
        privateKey: this.config[import_typescript_common.NANO_ACCOUNT_PRIVATE_KEY_PROPERTY],
  ...
  setNanoAccountPrivateKey(privateKey) {
    ...
    if (this.config[import_typescript_common.NANO_ACCOUNT_PRIVATE_KEY_PROPERTY]) {
      throw new Error(
        `[${ERROR_CONFIG}] Nano account private key already set. Manually clear with clearNanoAccountPrivateKey() first.`
      );
    }
    this.config[import_typescript_common.NANO_ACCOUNT_PRIVATE_KEY_PROPERTY] = privateKey;
  }
  /**
   * Clears the stored Nano account private key.
   */
  clearNanoAccountPrivateKey() {
    this.config[import_typescript_common.NANO_ACCOUNT_PRIVATE_KEY_PROPERTY] = void 0;
  }
  /**
